...
<template>
  <div>
    <h1 class="text-lg font-bold text-[#f0f0f0] mb-4">Comparar Pilotos</h1>
    ...
    <div v-if="d1 && d2" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="rounded-xl bg-[#0f0f0f] border border-[#1f1f1f] overflow-hidden">
          ...
          <div class="p-4 text-center">
            ...
            <p class="text-[10px] text-[#444] uppercase">puntos</p>
          </div>
        </div>
        <div class="rounded-xl bg-[#0f0f0f] border border-[#1f1f1f] overflow-hidden">
      ...
      <div class="rounded-xl bg-[#0f0f0f] border border-[#1f1f1f] p-5 space-y-4">
        <CompareStatBar label="Puntos" :value-a="d1.points" :value-b="d2.points"
          :color-a="getTeamColor(d1.constructorName || '')" :color-b="getTeamColor(d2.constructorName || '')" />
        <CompareStatBar label="Victorias" :value-a="d1.wins" :value-b="d2.wins"
          :color-a="getTeamColor(d1.constructorName || '')" :color-b="getTeamColor(d2.constructorName || '')" />
        <CompareStatBar label="Posicion" :value-a="d1.position" :value-b="d2.position"
          :color-a="getTeamColor(d1.constructorName || '')" :color-b="getTeamColor(d2.constructorName || '')" lower-is-better />
      </div>
    </div>

    <div v-else class="text-center py-20">
      <p class="text-xs text-[#444]">Selecciona dos pilotos para comparar</p>
    </div>
  </div>
</template>
